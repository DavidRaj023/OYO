...
class MainController {
  async createAdmin(req, res){
    try {
        if(req.body != null) {
          ...
          const email = await util.check('email', admin.email);
  ...
  async adminLogin(req, res){
    try {
        if(req.body != null) {
          const admin = req.body;
          const emailsList = await util.check('email', admin.email);
          ...
          util.updateToken(adminDetails.email, token);
  ...
  async createOwner(req, res){
    try {
      ...
      //   const password = await bcrypt.compare(admin.password, emailsList[0].password);
      //   if(password){
      //     const token = await util.generateAuthToken(emailsList[0].email);
      //     console.log(token);
      //     res.status(200).send({
      //       "message": `Welcome ${emailsList[0].name} `,
      //       "token": token
      //     });
      //   }
      // } else {
